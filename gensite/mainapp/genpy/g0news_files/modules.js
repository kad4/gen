...
ws.modules = {
    ...
    pictureGallery: function(selector) {       
        glow.dom.get(selector).each(function() {
            ...
            if (gallerySize[0] == "pg-320x180") {
                //hides captions for 320x180 pg - live events
                showCaptions = false;
            }
            ...
            // Assign an ID to the gallery if it is missing one
             ...
             galleryNode.get('> .content > ul > li').each(function(count) {
                var imageNode = glow.dom.get(this),
                    imageItem = {
                      ...
                      caption: imageNode.text()
                                        /*
					  size: function() {
						var pictureSize = imageItem.src.match(/([0-9]+)x([0-9]+)/),
							pictureWidth = parseInt(pictureSize[1]),
							pictureHeight = parseInt(pictureSize[2]),
							newImageNode = '',
							newChefSize = '144x81'; //Landscape image - default size
						
						if (pictureWidth == pictureHeight) { //Square image
							imageNode.get('img').attr('width', '92').attr('height', '');
							newChefSize = '92x70';
						} else if (pictureWidth < pictureHeight) { //Portait image
							imageNode.get('img').attr('width', '').attr('height', '49');
							newChefSize = '66x49';					
						}
						return newChefSize;
					  }
				...
				*/
				//console.log(imageNode.get('img'));
				//imageNode.html(); //imageItem.size //.replace
				//-------------
				...
				//var tempImageNode = ws.utils.outerHtml(imageNode.get('img')); //imageNode.get('img').html();				
				//newImageNode = tempImageNode.replace("144x81", newChefSize);
				
                //thumbsHtml += '<li><a class="pos-'+(count+1)+'" href="#" style="text-align: center; z-index: 100">'+ws.utils.outerHtml(imageNode.get('img'))+'</a></li>';    
				var tempImg = '<li><a class="pos-'+(count+1)+'" href="#">'+ws.utils.outerHtml(imageNode.get('img'))+'</a></li>'; 
				//thumbsHtml += '<li><a class="pos-'+(count+1)+'" href="#" style="text-align: center; z-index: 100">'+newImageNode+'</a></li>';
             ...
             // adding hidden caption scenario
             var footerNav;
             if (showCaptions) {
                ...
                var footerNav = '<div class="footer-nav blq-clearfix"><p class="image-counter"></p>' 
                           + '<ul class="controls"><li><a class="hideshow-captions"><span>'+vocab.showCaptions+'</span></a></li><li><a class="startstop-slideshow"><span>'+vocab.startSlideShow+'</span></a></li></ul></div>';
             }
...
//	    		 }
                ...
                if (!galleryNode.get('.content a.cta').length) {
                 ...
                 //on startuphides captions if required         
                 if (showCaptions == false) {
                    galleryNode.get('.caption').css('opacity','0');
                 }
             }                       
             
             // IE6 PNG Alpha Fix
             if ((glow.env.ie == 6) && showCaptions) { 
                captionOpacity = 0.75; 
    ...
    timeAgo: function(cssSelector) {
		...
		var rollingnewsThresholdHours = ws.config[ws.env.service].rollingnewsThresholdHours || 5;
                ...
                // live-events threshold
		var liveeventsThresholdHours = ws.config[ws.env.service].liveeventsThresholdHours || 12;
		...
		*/
		...
		var selector = (cssSelector) ? cssSelector : ws.selectors.timeAgo;
        ...
        $(selector).each(function() {						
			...
			if (lstpub.length > 0) { 
				...
				if (serverDistance !=0 && !serverDistance) { $(this).css("display", "none").css("visibility", "hidden"); }
			}
			...
			if (ws.utils.getParentWithClass(this, 'bx-rollingnews').length > 0 || $("body").hasClass("genre-rolling_news") ) {
				detailedThresholdHours = rollingnewsThresholdHours;
                        ...
                        // live events threshold
                        if( $("body").hasClass("live-event") ) {
                            detailedThresholdHours = liveeventsThresholdHours;
                            displayTimeAfterThreshold = true;
			    displayDateAfterThreshold = true;
                        }
			...
			if (!defaultDisplayTimeAfterThreshold) {
				displayTimeAfterThreshold = false; 
                    ...
                    if( $("body").hasClass("live-event") && serverDistance <= 0 ) {
                        serverDistance = serverDistance + (60 * 1000);
                    }
		...
		if (!ws.env.timeAgoIncrActive) {
            ...
            setInterval(ws.modules.timeAgo, ws.modules.timeAgo.settings.refreshMillis); 		
            ws.env.lastTime  = (new Date()).getTime(); 
        }

	    function timeAgoIncr() {	
            var currentTime = (new Date()).getTime(); 
            var timeAgoRefresh = ws.modules.timeAgo.settings.refreshMillis; 
            if (currentTime > (ws.env.lastTime + timeAgoRefresh*1.1)) {  
                var delay = currentTime - ws.env.lastTime; 
                ws.env.serverDate = new Date(ws.env.serverDate.getTime()+(delay)); 
            } else { 
                ws.env.serverDate = new Date(ws.env.serverDate.getTime()+(timeAgoRefresh)); 
            } 
            ws.env.lastTime = currentTime; 
            setTimeout(timeAgoIncr, timeAgoRefresh); 
		}
	...
	changeDegreesUnits: function(selector) {
		...
		var temp_coockie = ws.utils.readCookie ? ws.utils.readCookie('ckps_ws_weather_deg') : false;
		var currentDegrees = (temp_coockie && glow.dom.get('.w4').length)? ws.utils.readCookie('ckps_ws_weather_deg') : "cent";

		function setTemp (setTo) {
			if (setTo == "fahr") {
				glow.dom.get('.w4').addClass('w4-fahr');
				glow.dom.get('.w4').removeClass('w4-cent');							
				currentDegrees = "fahr";
				ws.utils.readCookie ? ws.utils.setCookie( 'ckps_ws_weather_deg','fahr',365) : '';
			} else {						  
				currentDegrees = "cent";	
				glow.dom.get('.w4').addClass('w4-cent');
				glow.dom.get('.w4').removeClass('w4-fahr');				
				ws.utils.readCookie ? ws.utils.setCookie( 'ckps_ws_weather_deg','cent',365) : '';
			}
		};
		setTemp(currentDegrees);

		if (selector) {
			glow.dom.get(selector).each(function() {
				glow.events.addListener(this, 'click', function(e) {
					var toogle = (currentDegrees == "cent") ? "fahr" : "cent";
					setTemp(toogle);				
					e.preventDefault();					
				});		
			});				
...
//			glow.dom.get(selector).removeClass(rnClassOn);
	...
	socialBookmarkPanel: function(selector) {		
		// scan document tools, if there are more than just facebook, twitter and googleplus add share icon with info panel to show the rest	
		...
		glow.dom.get(selector).each(function(){			
			...
			glow.dom.get(this).get('li').each(function(){		
				...
				if (!glow.dom.get(this).hasClass('facebook') && !glow.dom.get(this).hasClass('twitter') && !glow.dom.get(this).hasClass('googleplus') && !glow.dom.get(this).hasClass('email') && !glow.dom.get(this).hasClass('print') && !glow.dom.get(this).hasClass('report-error')) {					
			...
			if (!extraSocialBookmarkLinks == '') {				
				...
				//insert shared link and icon after the googlplus icon				
				glow.dom.get(this).get('.googleplus').after(shareLink);				
				...
				var panelContent = '<h2 class="hd">' + shareTitle + '</h2><div class="social-bookmark-content">' + helpLink + '<ul>' + ws.utils.outerHtml(glow.dom.get(this).get('.facebook')) + ws.utils.outerHtml(glow.dom.get(this).get('.twitter')) + ws.utils.outerHtml(glow.dom.get(this).get('.googleplus')) + extraSocialBookmarkLinks + '</ul></div>';
		...
		glow.events.addListener(twitterLink, 'click', function(e) {
			...
			var json="";
			var pageTitle=encodeURIComponent(glow.dom.get("h1").text());
				...
				useCache:true,
			  	onLoad: function(response) {
					...
					if (json.url.length > 0) {
						...
						var newLink = "http://twitter.com/?status="+pageTitle+" "+json.url;
	...
	liveEventsShare : function(selector) {
		if (selector == undefined) {
			var selector = ".live-text-module-share";
		}

		getShortUrl = function(item) {
			//Generate Link
			if (ws.env.isReallyLive == false) {
				var tunnel_for_dev_publish = "/cgi-bin/tunnel.pl?http://www.bbc.co.uk";
			} else {
				var tunnel_for_dev_publish = "";
			}
			var live_text_item = glow.dom.get(item).ancestors()[3];
			var item_id = live_text_item.id;
			var item_url = "http://" + document.location.host + document.location.pathname + "%23" + item_id;
			var shrinkRequest = tunnel_for_dev_publish + "/modules/share/service/shrink?url="+item_url+"&appid="+ws.env.servicePath;
			...
			glow.net.get(shrinkRequest, {
				...
				onLoad: function(response) {
					json = (glow.lang.trim(response.text()).length > 0) ? response.json() : '';
					if (json.url.length > 0) {
						onShortUrlReceived(json.url, item, item_url, item_id);
					}else{
						onJsonError(item, item_url, item_id);
					}
				},
				onError: function() {
					onJsonError(item, item_url, item_id);
		...
		onShortUrlReceived = function(url, item, long_url, item_id) {
			//populates the list 
			var social_list = glow.dom.create("<ul class=\"share-this-tools\"></ul>");
			var facebook_element = generateFacebookLi(long_url,item);
			facebook_element.appendTo(social_list);
			var twitter_element = generateTwitterLi(url, item);
			twitter_element.appendTo(social_list);
			var short_url_element = generateShortUrlLi(url);
			short_url_element.appendTo(social_list);
			var current_item = glow.dom.get(item).get(" .le-social-tools-inner");
			social_list.appendTo(glow.dom.get(current_item));			
		}

		onJsonError = function (item, long_url, item_id) {
			//populates the list without the short url 
			var social_list = glow.dom.create("<ul class=\"share-this-tools\"></ul>");
			var facebook_element = generateFacebookLi(long_url,item);
			facebook_element.appendTo(social_list);
			var twitter_element = generateTwitterLi(long_url, null);
			twitter_element.appendTo(social_list);
			var current_item = glow.dom.get(item).get(" .le-social-tools-inner");
			social_list.appendTo(glow.dom.get(current_item));				
		}

		generateTwitterLi = function(url, item) {
			var twitter_label = ws.vocab.dictionary.twitter ? ws.vocab.dictionary.twitter : "Twitter";
			var item_text = "";
			if (item != null) {
				var live_text_item = item.ancestors()[3];
				if (glow.dom.get(live_text_item).get('.body').length > 0) {
					item_text = ws.utils.truncate(glow.dom.get(live_text_item).get('.body').text(), 110 ,'...');				
				} else {
					item_text = ws.utils.truncate(glow.dom.get(live_text_item).get('.teaser_wrapper').text(), 110 ,'...');				
				}
				item_text = glow.lang.trim(item_text);	
				item_text = item_text.replace(/#/g,'%23') + " ";
			}	
			var twitter_url ="http://twitter.com/?status="+item_text + url; 
			var twitter_anchor = glow.dom.create("<a href=\""+twitter_url+"\" class=\"share-this-anchor\"><span class=\"st-icon\"></span>"+twitter_label+"</a>");
			var twitter_list = glow.dom.create("<li class=\"share-this-li twitter\"></li>");			
			twitter_anchor.appendTo(glow.dom.get(twitter_list));
			glow.events.addListener(twitter_anchor, 'click', function(e) {
				popMeUp(twitter_url, e);
			})	
			return twitter_list;
		}

		generateShortUrlLi = function(url) {
			var short_url_list = glow.dom.create("<li class=\"share-this-li short_url\"><span class=\"st-icon\"></span><p class=\"short-url-content\">"+url+"</p></li>");
			glow.events.addListener(short_url_list, 'click', function(e) {
				e.stopPropagation();
			})	
			return short_url_list;
		}

		generateFacebookLi = function(long_url, item) {
			var facebook_label = ws.vocab.dictionary.facebook ? ws.vocab.dictionary.facebook : "Facebook";
			//var facebook_title = glow.dom.get("h1").text().replace(/\s{2,}/g, ' ');
			//var facebook_url = "http://www.facebook.com/dialog/feed?app_id=58567469885&redirect_uri=http%3A%2F%2Fwww.bbc.co.uk%2Fmodules%2Fsharetools%2Fcallback%3Fst_cb%3Dfacebook%23state%3Dfeed&display=popup&link="+long_url+"&name="+facebook_title+"&locale="+ws.env.lang+"&description=description";
			var facebook_url="http://www.facebook.com/sharer.php?u="+long_url;
			var facebook_anchor = glow.dom.create("<a href=\""+facebook_url+"\" class=\"share-this-anchor\"><span class=\"st-icon\"></span>"+facebook_label+"</a>");
			var facebook_list = glow.dom.create("<li class=\"share-this-li facebook\"></li>");			
			facebook_anchor.appendTo(glow.dom.get(facebook_list));
			glow.events.addListener(facebook_anchor, 'click', function(e) {
				popMeUp(facebook_url, e);
			})	
			return facebook_list;
		}

		insertHtmlStructure = function(target) {
			var share_this = ws.vocab.dictionary.shareThis ? ws.vocab.dictionary.shareThis : "Share this";
			var social_container = glow.dom.create("<div class=\"le-social-tools-cont \"><span class=\"le-social-top-arrow\"></span><div class=\"le-social-tools-inner\"><h4>"+share_this+"<span class=\"le-social-close st-icon\"></span></h4></div></div> ");
			social_container.appendTo(target);
		}

		//I know, we already have the pop up module, but I can't specify a selector with that and I don't want it to run for all the anchor all the time (also I need e.stopPropagation)
		popMeUp = function(url, e) {
			popupWin = window.open(url,null,'width=750,height=350,scrollbars=no,toolbar=no,personalbar=no,location=no,directories=no,statusbar=no,menubar=no,resizable=yes,status=no,left=60,screenX=60,top=100,screenY=100'); 
           	popupWin.focus();
			e.stopPropagation();
           	e.preventDefault();
		}
		...
		glow.dom.get(selector).each(function(){
			...
			var share_text = ws.vocab.dictionary.share ? ws.vocab.dictionary.share : "Share";
			var share_text_label = glow.dom.create("<span class=\"le-share-label\"><span class=\"st-icon\"></span>" + share_text +"</span>");
			item.append(share_text_label);
			glow.events.addListener(item, 'click', function(e) {
				var live_text_item = glow.dom.get(glow.dom.get(item).ancestors()[3]);
				if (live_text_item.hasClass('live-text-module-element-active') == false) {
					var already_active_items = glow.dom.get(".live-text-module-element-active");
					already_active_items.removeClass('live-text-module-element-active');
				}
				live_text_item.toggleClass('live-text-module-element-active');
				if (item.children().length < 2) {
					insertHtmlStructure(item);
					getShortUrl(item);
				} 
			});
		})
