    ...
    emp: function(selector) {         
        glow.dom.get(selector).each(function() {
            var node        = glow.dom.get(this),                
                ...
                isLive      = ws.utils.getParentWithClass(this, 'av-live').length> 0 ? true : false,
			...
			// Add preroll to enabled sites
			ws.env.preEnabled = false;
			if (
				ws.env.showPre
				&& !isLive 
				&& ws.utils.readCookie('ckps_ws_pr') == null
				&& node.hasClass("ws-pre") 
				&& format == 'video' 
				&& node.attr('rel').match('/[0-9]{4}/[0-9]{2}/emp/')
				&& node.attr('rel').match('.emp.xml') 
				) {
				playlist = node.attr('rel').replace('/emp/','/emp/pre/');
				ws.env.preEnabled = true;

				//only set cookie after some time viewing video
				setTimeout(setAdDisplayFrequency,10*1000);
			}
			
			// show one preroll every 5 mins when enabled
			function setAdDisplayFrequency() { 
				ws.utils.setCookieMinutes('ckps_ws_pr','1',5);
			}
	    ...
	    else if (bodyNode.hasClass('live-event')) {
                if (node.parent().hasClass('teaser')) {
                    //bespoke size for lateral EMP
                    size = '336x189';
                } else {
                    size = '560x315'; 
             	} 
	    }
	    else if (bodyNode.hasClass('index') || bodyNode.hasClass('topic-cluster')|| bodyNode.hasClass('cluster')|| bodyNode.hasClass('story')) {			
		...
		if (node.parent().hasClass('ts-384emp')) { size = '384x216';} 
            ...
            if (format == 'audio') { 
				...
				showButtons = false; // mr: all hide popup for all audio due to emp bug
			}
            if (format == 'video') { 
				if (empVersion <= 2){
            ...
            // COMPANION ADS - this will only fire when the playlist has been redirected to the preroll xml, and on video emps
            if (playlist.match('/pre/')) {
	            if (typeof bbcdotcom !== 'undefined' && typeof bbcdotcom.av !== 'undefined' && typeof bbcdotcom.av.emp !== 'undefined') {
	                var companionMediaId = nodeId.replace('emp-','');
	                glow.dom.get('#bbccom_companion').attr('id', 'bbccom_companion_' + companionMediaId);
	
	                if (typeof bbcdotcom.av.emp.adverts !== 'undefined'){
	                /* enable companions */
	                    embeddedMedia.eachWrite(bbcdotcom.av.emp.adverts.playerBeforeEachWrite);
	                }
	                if(typeof bbcdotcom.av.emp.events !== 'undefined'){
	                /* enable analytics & display companions */
			...
			var title = glow.dom.get(this).get(".emp-title").html() || null,
			duration = glow.dom.get(this).get(".emp-duration").html() || null;
            ...
            // Keep track of any playing EMPs on the page (any changes here need to be duplicated in preroll fix below - emp.onMediaPlayerInitialisedPre)
...
// #############  START PREROLL FIX SiteCatalyst s_code.js - Duplicate of above and needs to be the same (emp.onMediaPlayerInitialised) ###############
	emp.onMediaPlayerInitialisedPre = function() {
		var obj = {
			mediaName : title, //	playlist				
			mediaId: nodeId.replace('emp-',''),					
			avType: format,
		...
		// Maually trigger start
		if (ws.env.documentType == 'av_instance' ) {
			emp.onPlaylistStarted();
		}
	}
	emp.onMediaPlayerPreInitialised = function() {
		if(typeof emp.onMediaCompleted!='undefined'){ // if the media player has finished loading
			emp.onMediaPlayerInitialisedPre();
		}
		else {
			if (ws.env.preCount<20) { //if the counter has been running for less than 20 tries
				setTimeout(emp.onMediaPlayerPreInitialised,3*1000); //wait 3 secs and try running again
			}
			ws.env.preCount++; //increment the counter
		}
	}
	if (ws.env.preEnabled) {
		ws.env.preCount=0; //set counter
		emp.onMediaPlayerPreInitialised(); // load emp.onMediaPlayerPreInitialised function 
	} 
// ############# END PREROLL FIX SiteCatalyst s_code.js ###############
